...
const PostDetails = async ({
  ...
  const blog = blogsData.find((blog) => blog.id === parseInt(slug));
  ...
  return (
    <div>
      ...
      {blog && (
        <div>
          <h2 className="text-2xl my-3">{blog.title}</h2>
          <p className="py-5 text-xl">{blog.author}</p>
          <p className="pb-10">{blog.content}</p>
        </div>
      )}
    </div>
  );
